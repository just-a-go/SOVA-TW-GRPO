...
def evaluate(model_name, batch_size, dataset_path=None, output_path=None):
    """
    ...
    try:
        ...
        model_path = model_name if os.path.isabs(model_name) else os.path.join(private_data_root, model_name)
        ...
        if output_path is None:
            output_path = os.path.join(
                ".", "logs", dataset_name, "test", base_path,
                f"{os.path.basename(model_path)}_eval.json"
            )
...
def main():
    ...
    parser.add_argument("--dataset_path", type=str, default=None,
                      help="Optional explicit path to the CLEVRER validation JSON")
    parser.add_argument("--output_path", type=str, default=None,
                      help="Optional explicit path for evaluation results")
