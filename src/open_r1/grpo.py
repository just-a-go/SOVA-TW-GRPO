...
class GRPOScriptArguments(ScriptArguments):
    ...
    Args:
        reward_funcs (`list[str]`):
            Reward functions: 'accuracy', 'origin_accuracy', or 'format'.
    """

    reward_funcs: list[str] = field(
        default_factory=lambda: ["accuracy", "format"],
        metadata={"help": "Reward functions: accuracy, origin_accuracy, format"},
    )
...
def origin_accuracy_reward(completions, solution, **kwargs):
    """Return one for a fully correct answer and zero otherwise."""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    current_time = datetime.now().strftime("%d-%H-%M-%S-%f")
...
def main(script_args, training_args, model_args):    
    ...
    trainer = trainer_cls(
        ...
        use_sova=model_args.use_sova,
        sova_mode=model_args.sova_mode,
        sova_lambda_positive=model_args.sova_lambda_positive,
        sova_lambda_negative=model_args.sova_lambda_negative,
        sova_positive_virtual_total_reward=model_args.sova_positive_virtual_total_reward,
        sova_negative_virtual_total_reward=model_args.sova_negative_virtual_total_reward,
        ngrpo_num_iterations=model_args.ngrpo_num_iterations,
